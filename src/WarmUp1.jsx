      ...
      <div style={rowStyle}>
        ...
        {fracMode === 'num' && (
          <button onClick={onSlash} style={{
            padding: '4px 6px', minWidth: '52px', flex: 1, maxWidth: '66px',
            borderRadius: '8px',
            ...
            <span style={{ fontSize: '11px', borderBottom: '1.5px solid #dd88ff', paddingBottom: '1px', minWidth: '18px', textAlign: 'center' }}>□</span>
            <span style={{ fontSize: '9px', color: '#dd88ff', marginTop: '1px' }}>↓</span>
            <span style={{ fontSize: '11px', minWidth: '18px', textAlign: 'center' }}>□</span>
          </button>
        )}
        {/* 通常時の指数ボタン */}
        {!fracMode && (
        ...
        {!fracMode && (
          <button onClick={onFrac} style={{
            ...
            <span style={{ fontSize: '11px', borderBottom: '1.5px solid #dd88ff', paddingBottom: '1px', minWidth: '18px', textAlign: 'center' }}>a</span>
            <span style={{ fontSize: '11px', minWidth: '18px', textAlign: 'center' }}>b</span>
          </button>
        )}
      </div>
      {/* 消去・Enter行 */}
...
export default function WarmUp1() {
  ...
  const [queue, setQueue]         = useState(allProblems)
  ...
  const handleKey = (val) => {
    ...
    setMessage('')
    if (fracState) {
      // 分母入力中に演算子が来たら分数を自動確定してtokensに追加
      const operators = ['+', '-']
      if (operators.includes(val) && fracState.phase === 'den' && fracState.den !== '') {
        setTokens(ts => [...ts, { type: 'frac', num: fracState.num, den: fracState.den }, { type: 'text', val }])
        setFracState(null)
        return
      }
      // 分子・分母ともに文字として追加（x-7 のような多項式分子も正しく入力できる）
      setFracState(fs => ({
        ...fs,
  ...
  const handleEnter = () => {
    ...
    const alts = (problem.altAnswers || []).map(normalize)
